...
- **기본 흐름**
  1. 서재가 빈 상태로 열림 → 빈 상태 안내 + "새 소설" · "JSON 가져오기" · "샘플 소설로 둘러보기" ([onboarding.md](./onboarding.md))
  2. 작가가 "새 소설" → 제목 입력(필수), 장르·소개는 선택
  3. 소설 생성과 함께 보드 1개, 기본 사전 분류 6개(캐릭터 / 사건 / 장소 / 세력·조직 / 아이템 / 세계관 설정)와 기본 템플릿, 기본 스토리 라인 3개(메인 / 서브 / 사이드) 생성
  4. 첫 소설이 생기는 순간 앱이 `navigator.storage.persist()`를 1회 요청, 결과 기록 (샘플·가져오기로 생긴 경우도 동일)
  5. 소설 작업공간의 보드 화면으로 이동 (시간축 0 눈금이 화면 중앙 근처). 보드 요소가 0개인 동안 안내 카드 표시(사건 · 캐릭터 상태 · 사전 3단계), 첫 요소를 놓으면 사라짐
- **대안·예외**
  - "샘플 소설로 둘러보기" → 앱에 포함된 샘플 JSON을 UC-05와 같은 경로로 가져와 새 소설로 추가 → 채워진 보드로 이동. 이후 일반 소설과 동일(편집·삭제 자유)
