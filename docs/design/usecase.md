...
flowchart LR
    ...
    F1["<b>F1 보드</b><br/>(배치 편집은 태블릿 이상)<br/>UC-10 사건 배치<br/>UC-11 기간·시점 조절<br/>UC-12 캐릭터 상태 기록<br/>UC-13 캐릭터별 정렬<br/>UC-14 눈금 라벨·간격·삽입<br/>UC-15 구간 접기<br/>UC-16 미정 영역<br/>UC-17 포스트잇·텍스트<br/>UC-18 프레임<br/>UC-19 연결선<br/>UC-20 선택·복사·실행 취소<br/>UC-21 둘러보기·터치<br/>UC-22 보드·위키 오가기<br/>UC-23 스토리 라인"]:::mvp
    F4["<b>F4 위키</b><br/>UC-30 분류 관리<br/>UC-31 문서 작성<br/>UC-32 템플릿<br/>UC-33 @ 링크·역링크<br/>UC-34 표·검색<br/>UC-35 문서 삭제"]:::mvp
...
flowchart LR
    ...
    UC22["UC-22 보드·위키 오가기"] -->|"패널에서 편집"| UC31
...
| UC-22 | 보드 ↔ 위키 오가기 | F1·F4 | ✅ | 전체 (드래그 생성은 태블릿 이상) |
| UC-23 | 스토리 라인 지정 · 관리 (메인·서브·사이드) | F1·F4 | ✅ | 전체 (블록 메뉴 지정은 태블릿 이상) |
| UC-30 | 위키 분류 관리 | F4 | ✅ | 전체 (드래그 정렬은 태블릿 이상) |
| UC-31 | 위키 문서 작성 | F4 | ✅ | 전체 |
| UC-32 | 분류 템플릿 설정 | F4 | ✅ | 전체 |
| UC-33 | `@` 링크 · 역링크 | F4 | ✅ | 전체 |
| UC-34 | 표 보기 · 검색 | F4 | ✅ | 전체 |
| UC-35 | 위키 문서 삭제 | F4 | ✅ | 전체 |
| UC-40 | 자동 저장 | F6 | ✅ | 전체 |
...
- **기본 흐름**
  ...
  4. 소설 생성과 함께 보드 1개, 기본 위키 분류 6개(캐릭터 / 사건 / 장소 / 세력·조직 / 아이템 / 세계관 설정)와 기본 템플릿, 기본 스토리 라인 3개(메인 / 서브 / 사이드) 생성
...
- **기본 흐름**
  ...
  3. 카드 클릭 → 마지막으로 보던 탭(보드/위키)과 보드 화면 위치로 복귀
...
- **기본 흐름**
  ...
  - 복제: 소설 전체(보드·위키·이미지)를 새 ID로 복사, 제목 뒤 "(사본)"
...
- **기본 흐름**
  ...
  3. **항상 새 소설로 추가** (모든 ID 재발급, 위키 본문 멘션의 문서 ID도 함께 변환) → 같은 파일을 두 번 가져와도 충돌 없음
  4. 서재에 새 카드 표시
- **대안·예외**
...
- **기본 흐름**
  ...
  3. 사건 분류에 위키 문서 자동 생성, 블록 제목 입력 = 문서 제목
...
- **대안·예외**
  ...
  - 사건 블록 삭제 → 사건 문서는 남음(위키에서 다시 보드로 끌어 배치 가능). 단, 블록과 함께 자동 생성된 뒤 내용이 비어 있는 문서는 함께 삭제
...
### UC-22 보드 ↔ 위키 오가기
- **기본 흐름**
  1. 사건·상태 블록의 상세 버튼 → 우측 위키 패널(태블릿: 오버레이, 모바일: 하단 시트)에 문서 열림
  2. 패널에서 문서 수정 → 블록 제목·태그 즉시 반영
  3. 위키 패널의 사건 문서를 보드로 끌기 → 사건 블록 생성(이미 보드에 있으면 그 블록으로 이동)
...
- **기본 흐름**
  ...
  - 위키 사건 문서의 "라인" 선택 칸에서도 지정 (보드 블록이 없는 사건 문서도 가능)
...
| UC-52 메모 관리 · 변환 (F5) | 소설별 메모 목록·핀 고정, 포스트잇 ↔ 메모, 메모 → 위키 문서 |
