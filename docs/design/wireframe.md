...
MVP 화면(서재 · 보드 · 사전)의 구조와 배치. [유스케이스](./usecase.md)의 흐름이 화면에서 빠짐없이 지원되는지 대조하는 기준으로 사용.
...
- **빈 상태 · 온보딩**: 규칙·문구는 [onboarding.md](./onboarding.md), 프레임은 L-2 · B-9 · W-7
...
flowchart LR
    ...
    BOARD <-->|"탭 · Alt+1/2"| WIKI["<b>W-1 사전</b>"]
    BOARD -->|"블록 상세 · Enter"| PANEL["B-2 사전 패널"]:::sub
    PANEL -->|"사전에서 열기"| WIKI
...
| 작업공간 상단 바 (보드·사전 공통) | ← 서재 · 소설 제목 · 탭(보드/사전) · 저장 상태(저장 중/저장됨/저장 실패) · 빠른 이동 검색칸(`Ctrl+K`) · `?` 단축키 도움말 · ⋯ 메뉴(내보내기 · 보드 설정 · 소설 정보) |
| 보드 | 전체 화면 캔버스 + 상단 중앙 도구 모음 + 우하단 줌·미니맵 + (열면) 우측 사전 패널 |
| 사전 | 최대 1280px 중앙: 좌측 분류 트리 240px + 우측 문서 영역 |
...
| L-2 | 빈 상태 | 새 소설 · 가져오기 · 샘플 소설로 둘러보기(가져오기 경로 재사용), IndexedDB 사용 불가 경고 배너(예외) | 01 |
...
| B-2 | 블록 선택 · 블록 메뉴 · 사전 패널 | 선택 강조 + 리사이즈 핸들, 블록 메뉴(색 · 태그 · 라인 · 상세 · 복제 · 삭제) + 라인 하위 메뉴, 우측 사전 패널(라인 · 시점 · 태그 · 관련 캐릭터 · 속성 · 본문), 패널 문서 → 보드 끌기, 박스 선택 | 10 · 11 · 20 · 22 · 23 · 40 |
...
| B-8 | 빠른 이동 | 사전 문서 · 시간축 시점 두 그룹 결과, 접힌 구간이면 펼치고 이동 | 21 · 34 |
| B-9 | 빈 보드 | 요소 0개일 때 안내 카드(사건 · 캐릭터 상태 · 사전 3단계), 도구 강조, 0 눈금 중앙, 필터로 전부 숨김 알림 | 01 · 23 |

## 4. 사전 — [wiki.html](./wireframes/wiki.html)
...
| W-7 | 빈 사전 · 빈 분류 · 검색 결과 없음 | 기본 분류 6개(0), 빈 분류 안내, 검색 0건(검색어로 새 문서), 빠른 이동 0건, 빈 역링크 · 보드 연동 | 01 · 31 · 34 |
...
| UC | 프레임 | UC | 프레임 |
|---|---|---|---|
| UC-01 첫 실행 · 첫 소설 | L-2 · L-3 · B-9 · W-7 | UC-19 연결선 | B-1 |
| UC-02 목록 · 열기 | L-1 | UC-20 선택 · 복사 · 실행 취소 | B-2 |
| UC-03 수정 · 복제 · 삭제 | L-1 · L-3 · L-4 | UC-21 둘러보기 | B-1 · B-7 · B-8 |
| UC-04 JSON 내보내기 | L-1 · L-4 · B-7 | UC-22 보드 ↔ 사전 | B-2 · W-1 |
...
- **사전 패널**: 폭 약 400px, 오버레이가 아니라 캔버스를 밀어냄. 상세 버튼 또는 `Enter`로 열림 (더블클릭은 인라인 편집)
- **상태 블록 입력**: 놓은 직후 캐릭터 선택 → 유형별 입력 팝오버. 입력은 자동 저장, "완료"는 닫기만
- **레인 모드**: 상태 블록 지시선 생략 (레인과 겹침), 레인 머리는 화면 왼쪽 고정
- **백업 알림**: 상단 바 아래 가로 배너 (보드·사전 공통)
- **저장 실패**: 상단 바 저장 상태를 눌러 다시 시도 · 내보내기, 공간 부족이면 L-5 대화상자
- **보드 설정**: ⋯ 메뉴 → 팝오버 (눈금 간격 · 스냅 · 축소 시 간략 표시)
- **사전 분류 화면**: 트리에서 분류 이름 클릭 → 탭 3개(문서 목록 · 표 · 설정). 표 보기·템플릿 설정의 진입점
- **사전 검색**: 트리 위 검색칸 → 결과는 문서 영역. `Ctrl+K`는 문서 + 시점 빠른 이동
- **문서 → 보드**: 문서 상단 "보드에서 보기 · N" (연결 블록 수)

## 7. 남은 일
- 태블릿(768–1023px) · 모바일(<768px) 와이어프레임 — 도구 모음 축소, 사전 패널 오버레이/하단 시트, 모바일 편집 제한 (A8)
