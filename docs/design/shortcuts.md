...
flowchart TB
    ...
    SCOPE -->|"사전 · 서재"| OTHER["공통 단축키만<br/>(1장)"]
...
| 키 | 동작 | 비고 |
|---|---|---|
| `Ctrl+K` | 빠른 이동: 사전 문서(제목·별칭)·눈금 라벨 검색 → 선택 시 문서 열기 / 보드 해당 시점으로 이동 | UC-21 "특정 시점으로 이동", UC-34 검색 |
| `Shift+/` (`?`) | 단축키 도움말 열기 | 이 문서의 표를 앱 안에 표시 |
| `Ctrl+S` | 저장 대화상자 대신 "자동 저장됨" 알림 | 습관적 입력 대응, 브라우저 저장 막음 (UC-40) |
| `Esc` | 한 단계 닫기 (1.1 순서) | |
| `Alt+1` / `Alt+2` | 작업공간 탭 전환: 보드 / 사전 | 소설 작업공간에서만 |
...
- 보드 실행 취소와 섞이지 않음: 사전 본문은 Tiptap 자체 기록, 보드 기록은 보드 캔버스 포커스에서만 (UC-20)
...
| UC-22 보드 ↔ 사전 | `Enter` 문서 열기, `Alt+1`·`Alt+2` |
