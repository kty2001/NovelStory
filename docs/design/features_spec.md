...
| 용어 | 정의 |
|---|---|
| 소설 (Novel) | 작품 1개 단위. 보드·위키·메모·회차의 최상위 |
...
| 위키 문서 (Wiki Doc) | 캐릭터·사건·장소·세계관 등 설정 문서 |
| 분류 (Category) | 위키 문서의 계층형 그룹 (캐릭터, 장소 등) |
| 속성 (Property) | 위키 문서의 키-값 정보 (나이, 소속 등) |
...
- 특정 시점 선택 시 해당 시점 기준 캐릭터 상태 요약 (생존 여부, 소속, 관계 등)
  - 계산: 위키 문서 속성 + 해당 시점까지의 상태 블록 속성 변경을 시간순으로 누적
  - 미정 영역 블록은 계산에서 제외
- 캐릭터별 상태 변화 이력 목록 (위키 캐릭터 문서에도 자동 표시)
...
| 화면 폭 | 소설 작업공간 구성 |
|---|---|
| 데스크톱 (≥1024px) | 상단 탭 · 전체 화면 보드 + 떠 있는 도구 모음 · 우측 위키 패널 |
| 태블릿 (768–1023px) | 도구 모음 축소, 위키 패널 오버레이 |
| 모바일 (<768px) | 하단 탭(보드 / 서술 순서 / 위키 / 메모), 위키는 하단 시트 |

- **모바일 지원 수준**: 보기 + 간단 편집
  - 가능: 보드 이동·확대, 블록 내용 수정, 위키·메모 편집
