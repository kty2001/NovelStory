...
### 사전 분류 색
사전 트리·링크 칩·표의 분류 표시 점에 사용. 캐릭터·사건은 보드 블록 색과 일치.
...
| `{colors.surface-soft}` | #faf5e8 | 사전 패널, 사이드 트리 배경 |
...
- **말줄임**: 블록 라벨은 1~2줄 말줄임(`line-clamp-2`), 전체 내용은 사전 패널에서 확인
...
### 화면 구성
- **보드**: 전체 화면 캔버스 + 상단 중앙에 떠 있는 도구 모음 + 우측 사전 패널(열고 닫기) + 우하단 미니맵·줌 컨트롤
- 보드 배경: `{colors.canvas}` + 옅은 점 격자(`{colors.hairline}`)
- 시간축: 캔버스 가로 기준선. 축 위 사건 영역, 축 아래 캐릭터 상태 영역
- **서재·사전·메모·서술 순서**: 최대 1280px 중앙 정렬
- **사전**: 좌측 분류 트리(240px) · 우측 문서 본문
...
| 이름 | 폭 | 주요 변화 |
|---|---|---|
| Mobile | < 768px | 하단 탭 내비게이션, 사전 패널은 하단 시트, 미니맵 숨김. 보드는 보기·블록 내용 수정만 (도구 모음·배치 편집 비활성) |
| Tablet | 768–1023px | 도구 모음 축소(아이콘만), 사전 패널 오버레이 |
| Desktop | 1024–1439px | 전체 화면 보드 + 떠 있는 도구 모음 + 우측 사전 패널 |
...
- 사전: 트리 + 문서 2단 → 모바일은 트리 화면 → 문서 화면 이동
- 블록 색·모양은 모든 폭에서 유지
